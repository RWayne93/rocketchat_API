...
class RocketChatRooms(RocketChatBase):
    ...
    async def rooms_get(self, **kwargs):
        ...
        return await self.call_api_get("rooms.get", kwargs=kwargs)
    ...
    async def rooms_info(self, room_id=None, room_name=None):
        ...
        if room_id is not None:
            return await self.call_api_get("rooms.info", roomId=room_id)
        if room_name is not None:
            return await self.call_api_get("rooms.info", roomName=room_name)
        raise RocketMissingParamException("room_id or roomName required")

    async def rooms_admin_rooms(self, **kwargs):
        """Retrieves all rooms (requires the view-room-administration permission)."""
        return await self.call_api_get("rooms.adminRooms", kwargs=kwargs)

    async def rooms_create_discussion(self, prid, t_name, **kwargs):
        """
