...
async def test_livechat_rooms(logged_rocket):
    livechat_rooms = await logged_rocket.livechat_rooms().json()
    assert livechat_rooms.get("success")
    assert "rooms" in livechat_rooms


async def test_livechat_inquiries_list(logged_rocket):
    livechat_inquiries_list = await logged_rocket.livechat_inquiries_list().json()
    assert livechat_inquiries_list.get("success")
    assert "inquiries" in livechat_inquiries_list


async def test_livechat_inquiries_take(logged_rocket):
    # TODO: Find a way of creating an inquiry to be able to properly test this method
    livechat_inquiries_take = await logged_rocket.livechat_inquiries_take(
...
async def test_livechat_users(logged_rocket):
    livechat_users = await logged_rocket.livechat_get_users(user_type="agent").json()
    assert livechat_users.get("success")
    assert "users" in livechat_users
    assert len(livechat_users.get("users")) == 0

    username = await logged_rocket.me().json().get("username")
    new_user = await logged_rocket.livechat_create_user(
    ...
    livechat_get_user = await logged_rocket.livechat_get_user(
        user_type="agent", user_id=new_user.get("user").get("_id")
    ).json()
    assert livechat_get_user.get("success")
    assert new_user.get("user").get("username") == username

    livechat_delete_user = await logged_rocket.livechat_delete_user(
        user_type="agent", user_id=new_user.get("user").get("_id")
    ).json()
...
async def test_livechat_visitor_minimal(logged_rocket):
    token = str(uuid.uuid1())
    new_visitor = await logged_rocket.livechat_register_visitor(token=token).json()
    assert new_visitor.get("success")
    assert "visitor" in new_visitor


async def test_livechat_visitor_room_and_message(logged_rocket):
    token = str(uuid.uuid1())
    name = str(uuid.uuid1())
    new_visitor = await logged_rocket.livechat_register_visitor(
        token=token, visitor={"username": name}
    ).json()
    assert new_visitor.get("success")
    assert "visitor" in new_visitor

    get_visitor = await logged_rocket.livechat_get_visitor(token=token).json()
    ...
    livechat_room = await logged_rocket.livechat_room(token=token).json()
    assert livechat_room.get("success")
    assert "room" in livechat_room

    livechat_message = await logged_rocket.livechat_message(
        token=token,
    ...
    livechat_messages_history = await logged_rocket.livechat_messages_history(
